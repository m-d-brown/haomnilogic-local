...
class OmniLogicAPI:
    ...
    async def async_set_chlorinator_enable(self, bow_id: int, enabled: bool | int) -> None:
        ...
        # We use bow_id as key for chlorinator enable state in this fake for simplicity
        self.state[f"chlor_enable_{bow_id}"] = bool(enabled)

    async def async_set_chlorinator_params(self, bow_id: int, system_id: int, timed_percent: int, **kwargs: Any) -> None:
        _LOGGER.debug("FakeOmniLogicAPI: async_set_chlorinator_params called (bow_id: %s, system_id: %s, timed_percent: %s)", bow_id, system_id, timed_percent)
        self.state[system_id] = timed_percent
